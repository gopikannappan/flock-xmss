...
fn main() {
    ...
    let sigs: Vec<_> = keys.iter().map(|kp| Signature {
    ...
    for r in 0..runs {
        ...
        let ok = verify_sound(&setup, &proof, &roots, &bits, &mut chv).is_ok();
        println!("  run {}: prove {:.3}s ({:.1} XMSS/s)  verify {:.1}ms ok={}",
                 r + 1, dt, k as f64 / dt, tv.elapsed().as_secs_f64() * 1e3, ok);
        assert!(ok);
    }
